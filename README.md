...
把这个文件夹压缩发给用户。用户把 `.env.example` 复制为 `.env`，填入网页右上角显示的 `WORKER_CLIENT_ID`，然后双击 `start_worker.bat` 即可运行采集端。采集端会复用用户本机浏览器登录态，负责 1688 采集和淘宝市场价采集。

采集端默认会优先打开用户系统默认浏览器。默认浏览器需要是 Chrome、Edge 或其他 Chromium 系浏览器；如果用户默认浏览器不兼容，可以在 `.env` 中手动填写：

```text
CRAWLER_BROWSER_EXECUTABLE_PATH=C:\Program Files\Google\Chrome\Application\chrome.exe
```
...
CRAWLER_USER_DATA_DIR=data/playwright_profile
CRAWLER_USE_DEFAULT_BROWSER=false
